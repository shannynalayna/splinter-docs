...
This guide explains how to remove a circuit by either disbanding or abandoning
a circuit, followed by  purging the circuit. Disbanding a circuit begins with
proposing to remove a circuit’s networking functionality. This allows all
members to agree before the circuit is disbanded and only available locally to
each member. A node’s administrator is able to abandon a circuit, breaking the
circuit’s networking ability for that node to send or receive messages on the
abandoned circuit. Whether a circuit is disbanded or abandoned, the result
removes a circuit’s networking capability. This allows a circuit to be purged.
A node administrator may purge a circuit once it has been disbanded or
abandoned in order to delete a circuit from state and remove any internal
service data.
...
either operation. The following describes each of these commands in more detail.
...
that supports member’s networking and to entirely delete its state contents.
The steps to fully delete a circuit include disconnecting the circuit's
networking and then removing the circuit's state. An individual node’s
administrator may choose to abandon a circuit, disconnecting the circuit’s
networking capability for that node. The process to disband a circuit involves
all circuit members agreeing to remove the circuit’s networking capability. A
circuit may be purged once it is deactivated, meaning the circuit's state and
internal service data is deleted. If any of this data still needs to be
accessed and cannot be deleted, do not purge the circuit or copy any important
information somewhere outside of Splinter.
...
circuit, whereas abandoning a circuit involves only a single node. The disband
command creates a situation where  circuit members are able to gracefully
shutdown the circuit. When a circuit is requested to be disbanded, a new
`CircuitProposal` is created with the same information in the active circuit,
but with a `circuit_status` of `Disbanded`. The `CircuitProposal` has a
`ProposalType` of `Disband`. At this point the original circuit has not been
affected and the new `CircuitProposal` with the updated `circuit_status` is
available to be voted on by circuit members. Note that the associated disband
proposal has the same circuit ID as the active circuit; the only change in
the circuit is the `circuit_status` field.
...
`Disbanded`. This also removes the peer connections associated with the members
of the circuit for each individual node. If the peers are connected via a
separate circuit, the peers will remain connected via that other circuit. If
the disbanded circuit was the only circuit connecting the peers, the peer
...
For more information on the `disband` CLI command, see the
 [`splinter-circuit-disband`](
{% link docs/0.5/references/cli/splinter-circuit-disband.1.md %}) man page.
...
For more information on the `abandon` CLI command, see the
 [`splinter-circuit-abandon`](
{% link docs/0.5/references/cli/splinter-circuit-abandon.1.md %}) man page.
...
* A splinter network with at least two nodes running and at least one circuit
  ...
  [Configuring Splinter Nodes](
  {% link docs/0.5/tutorials/configuring_splinter_nodes.md %})
...
1. Connect to the beta Splinter node to finish disbanding the circuit.
    ...
    ```console
    root@splinterd-beta:/# splinter circuit vote 01234-ABCDE --accept \
      --url http://splinterd-beta:8080 \
      --key <path_to_beta_private_key>
    ```
    ...
    ```console
    root@splinterd-beta:/# splinter circuit show 01234-ABCDE \
      --url http://splinterd-beta:8080 \
      --key <path_to_beta_private_key>
    Circuit: 01234-ABCDE
       Display Name: -
...
1. Connect to the alpha Splinter node to abandon the active circuit.
    ...
    ```console
    root@splinterd-alpha:/# splinter circuit abandon 01234-ABCDE \
      --url http://splinterd-alpha:8080 \
      --key <path_to_alpha_private_key>
    ```
    ...
    ```console
    root@splinterd-alpha:/# splinter circuit list --circuit-status abandoned \
      --url http://splinterd-alpha:8080 \
      --key <path_to_alpha_private_key>
    ID          NAME  MANAGEMENT MEMBERS
    01234-ABCDE -     example    beta;alpha
...
beta node must first abandon the circuit in order to purge the
circuit.
...
1. Connect to the alpha node to purge the circuit.
    ...
    ```console
    root@splinterd-alpha:/# splinter circuit purge 01234-ABCDE \
      --url http://splinterd-alpha:8080 \
      --key <path_to_alpha_private_key>
    ```
